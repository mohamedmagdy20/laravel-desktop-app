@extends('layout.app')
@section('title','إِلتزامات السيد رئيس اركان المشاة')
@section('content')
  ...
  <!--end breadcrumb-->
    <div class="card mt-3 shadow-sm p-3 mb-5 bg-white rounded">
      ...
      <div class="card-body">
        ...
        <div class="table-responsive rounded-2 shadow" >
          <table  dir="rtl" class="table table-striped table-bordered ">
            ...
            <tbody>
             ...
             @if ($item->date  == $today)
             <tr style="--bs-table-accent-bg: #018786 !important;color:#fff">
              <td>{{$index+1}}</td>
              <td>{{$item->date}}</td>
              <td>{{$item->day}}</td>
              <td>{{$item->name}}</td>
              ...
              <td>{{$item->note}}</td>
              <td>
                <a href="{{route('edit',$item->id)}}" class="btn btn-secondary btn-sm" ><i class="fa-solid fa-pen"></i></a> <!-- Update icon -->
